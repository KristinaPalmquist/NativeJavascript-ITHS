...
document.getElementById('nav-cities').style.fontWeight = 'bolder';
document.getElementById('nav-cities').style.color = 'black';
  ...
  .then((cities) => {
    ...
    cities.forEach((city) => {
      ...
      citiesResults.innerHTML +=
        ...
        <td class=popRow>${city.population.toLocaleString('sv-SE')}</td>
    </tr>`;
      // fill dropdown list
      const selectCity = document.getElementById('exampleFormControlSelect1');
      let citiesIdArray = [];
      selectCity.innerHTML += `<option value=${city.id}>${city.name}</option>`;
    ...
    selectElement.addEventListener('click', () => {
      ...
      editBtn.addEventListener('click', () => {
        ...
        fetch('https://avancera.app/cities/' + output, {
          body: JSON.stringify({
            id: chosenID,
            name: editedCityName,
            population: editedCityPop,
          }),
          headers: { 'Content-Type': 'application/json' },
          method: 'PUT',
        }).then(() => {
          document.getElementById('postRegistered').hidden = true;
          document.getElementById('putRegistered').innerHTML =
            sessionStorage.getItem('chosenName') +
            ' has been updated to ' +
            editedCityName +
      ...
      deleteBtn.addEventListener('click', () => {
        ...
        fetch('https://avancera.app/cities/' + chosenID, {
          ...
          document.getElementById('postRegistered').hidden = true;
          document.getElementById('putRegistered').hidden = true;
          document.getElementById('deleteRegistered').innerHTML =
            sessionStorage.getItem('chosenName') + ' has been deleted.';
...
registerBtn.addEventListener('click', () => {
  ...
  fetch('https://avancera.app/cities/', {
    ...
    document.getElementById('putRegistered').hidden = true;
    document.getElementById('deleteRegistered').hidden = true;
    document.getElementById('postRegistered').innerHTML =
      newCityName +
      ' has been registered with a population of ' +
      newCityPop +
      ' people.';
    document.querySelector('#updateBtnDiv').hidden = false;
  });
